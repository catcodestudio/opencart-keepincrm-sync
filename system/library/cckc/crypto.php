...
/**
 * At-rest encryption for the stored KeepinCRM API key. sodium_crypto_secretbox
 * when available, otherwise an HMAC-stream cipher; legacy plaintext is returned
 * as-is on decrypt so an upgrade never loses a key. The key is derived from
 * OpenCart path/db constants so a value encrypted in admin decrypts from
 * catalog/cron/cli.
 */
class Crypto {
	private const PREFIX_SODIUM = 'cckc1:';
	private const PREFIX_HMAC   = 'cckc2:';
	...
	private static function key(): string {
		...
		return hash('sha256', 'CatCodeKeepincrmSync|' . $material, true);
	}
